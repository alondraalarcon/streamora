...
import {
  fetchMovieShow,
  fetchMovieReviews,
  fetchSimilarMovieShow,
} from '@/app/services/moviesServices';
...
import MovieItem from '@/app/movies/components/MovieItem';
...
const TvPage = () => {
  ...
  const [show, setMovieShow] = useState<Show | null>(null);
  ...
  const fetchTvShowRecord = async (id: number) => {
    return withErrorHandling(async () => {
      const data = await fetchMovieShow(id);
      return data;
  ...
  const fetchTvShowYoutubeVideos = async (id: number) => {
    return withErrorHandling(async () => {
      const data = await fetchVideoUrl(id, 'movie');
  ...
  const fetchSimilarVideos = async (id: number) => {
    return withErrorHandling(async () => {
      const data = await fetchSimilarMovieShow(id);
      return data.results;
    }, 'Failed to load tv reviews.');
  };
  if (isLoading) {
            ...
            <div className="flex flex-col gap-2 mt-8">
              ...
              <div className="flex flex-row gap-1">
                <span className="text-sm font-light">
                 {show?.release_date?.substring(0, 4) ?? 'N/A'}
                </span>
                <span>
                  <Dot size={20} />
                </span>
